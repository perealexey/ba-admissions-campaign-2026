...
st.info(
    ...
    "желанности (M4) и не суммируется с обычным бюджетом.",
    icon="ℹ️",
)
...
with tab_program:
    ...
    if len(applicants_table):
        ...
        st.caption(
            ...
            f"«Приоритет (бюджет)» — уже сведён к одному значению на человека (правило "
            f"схлопывания бюджетной семьи: Бюджетные места + Отдельная квота + Особое право "
            f"считаются одной очередью, без задвоения)."
        )
    else:
