...
# Фон для st.info-врезок — ТЕМНЕЕ фона страницы (карточка "утоплена"), а не
# светлее, как было раньше (светло-синяя рамка на синем фоне читалась плохо).
# У тёмной карточки на светлом фоне сразу два плюса: она заметно отличается
# от фона визуально, и контраст белого текста внутри неё намного выше нормы
# WCAG (тёмный фон + светлый текст — надёжная пара, в отличие от светлый+светлый).
INFO_BOX_BACKGROUND_COLOR = "#284293"
# Левая полоса-акцент врезок — тот же фиолетовый, что primaryColor темы
# (.streamlit/config.toml), а не белый: белая полоса на синей заливке почти
# не читалась как акцент, просто как более светлый край.
INFO_BOX_ACCENT_COLOR = "#8B5CF6"
# Разделительные линии (st.divider, граница под вкладками) — на оттенок
# темнее фона страницы, а не светлее: тёмная линия лучше видна на светлом
# фоне, чем ещё один светлый оттенок поверх светлого.
DIVIDER_COLOR = "#3356BE"
...
def inject_theme():
    ...
    st.markdown(
        ...
        конкурса" и служебные st.info по ходу дашборда) — тёмная "карточка" на
        светлом фоне страницы (по образцу магистерского дашборда, у которого
        это получается бесплатно из тёмной темы Streamlit; здесь фон светлый,
        поэтому эффект приходится красить руками). */
        [data-testid="stAlertContainer"] {{
            background-color: {INFO_BOX_BACKGROUND_COLOR} !important;
            border-left: 4px solid {INFO_BOX_ACCENT_COLOR};
            border-radius: 8px;
        }}
        ...
        /* Разделительные линии — st.divider() и граница под панелью вкладок —
        на 1px толще и на оттенок темнее фона (см. DIVIDER_COLOR), иначе
        терялись на светлом фоне страницы. */
        hr {{
            border-top: 2px solid {DIVIDER_COLOR} !important;
        }}
...
st.info(
    ...
    "«сколько подано заявлений». Здесь предпринята попытка ответить на более важный: "
    "**насколько абитуриенты действительно хотят каждую программу** "
    "(как часто ставят её первым приоритетом) "
    ...
    "запасным вариантом.\n\n"
    "Данные охватывают конкурс по четырём кампусам: Москва, Санкт-Петербург, "
    "Нижний Новгород, Пермь."
)
st.info(
    "**Как пользоваться.**\n\n"
    "- **Базовая статистика** ниже — просто число заявлений по выбранным программам.\n"
    "- **«По программе»** — подробные показатели и рейтинг интереса.\n"
    "- **«Сравнить программы»** — две программы бок о бок.\n"
    "- **«По абитуриенту»** — все заявки одного человека.\n"
    "- Почти у каждого показателя есть кнопка «Подробнее» — там формула и пояснение "
    "простым языком."
)
...
st.info(
    ...
    "не входит в рейтинг интереса и не прибавляется к обычному бюджету."
)
...
with tab_program:
    ...
    with st.expander("Подробнее о метриках"):
        st.markdown(
            ...
            f"отдано под целевой приём (не общий конкурс). Не входит в рейтинг интереса (M4), "
    ...
    st.subheader("Рейтинг интереса к программам")
    st.caption(
    ...
    st.caption(
        ...
        "не напрямую как «эта программа интереснее для всех», как у массовых "
    ...
    with st.expander("Подробнее о метрике"):
        ...
        st.latex(r"\text{Интерес} = \dfrac{\text{cnt}_{p1} + k \cdot p_0}{n + k}")
        st.markdown(
        ...
        st.markdown(
            ...
            f"Это оправдано на данных: интерес по бюджету и по платному, посчитанные "
    ...
    ranked_display = ranked_display.rename(columns={
        ...
        "desirability_budget": "Интерес (бюджет)",
        "desirability_commercial": "Интерес (платное)",
        "desirability_combined": "Интерес (общий)",
        "target_quota_share_of_budget": "Доля целевой квоты от бюджета",
    })
    st.dataframe(
        ranked_display,
        width="stretch", hide_index=True, height=400,
        column_config={
            "Интерес (бюджет)": st.column_config.NumberColumn(format="%.3f"),
            "Интерес (платное)": st.column_config.NumberColumn(format="%.3f"),
            "Интерес (общий)": st.column_config.NumberColumn(format="%.3f"),
        },
    )
    ...
    if len(applicants_table):
        ...
        # Приоритеты/баллы остаются числовым типом (не строкой): клик по шапке
        # таблицы в Streamlit 1.50.0 пересортировывает по типу колонки —
        # строковая колонка сортируется лексикографически ("1, 10, 11, 2"), а
        # числовая — верно. Пропуски (подавался не всеми видами конкурса) при
        # этом рисуются буквально текстом "None" — известный баг рендерера
        # этой версии, но не так важно, как корректная сортировка
        # (см. CONTEXT_FOR_FUTURE.md).
        st.dataframe(applicants_table, width="stretch", hide_index=True, height=400)
        st.caption(
            f"Всего уникальных абитуриентов по этой программе: "
            f"{applicants_table['ID (Госуслуги)'].nunique()}. «None» — не подавался по этому виду конкурса. "
...
with tab_compare:
    ...
    if program_a is None or program_b is None:
        ...
        rows = {
            ...
            "Интерес (бюджет)": cmp["desirability_budget"].round(3),
            "Интерес (платное)": cmp["desirability_commercial"].round(3),
            "Интерес (общий)": cmp["desirability_combined"].round(3),
            "Ранг интереса": cmp["desirability_rank"],
        }
        ...
        if common_ids:
            ...
            # Колонки остаются Int64 (не строкой): клик по шапке таблицы в
            # Streamlit 1.50.0 пересортировывает по типу колонки — строковая
            # колонка сортируется лексикографически ("1, 10, 11, 2"), а Int64 —
            # верно. Пустой вид конкурса при этом рисуется буквально текстом
            # "None" — известный баг рендерера этой версии, но не так важно,
            # как корректная сортировка (см. CONTEXT_FOR_FUTURE.md).
            ...
            st.caption(
                ...
                "целевая квота). «None» — не подавался этим видом. Нумерация приоритетов "
