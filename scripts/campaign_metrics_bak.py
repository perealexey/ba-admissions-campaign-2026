...
def program_base(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # то же самое имя программы в разных филиалах (например «Медиакоммуникации»
    # в Москве и в СПб — это НЕ близнецы, обычная ситуация, программа есть в
    # обоих городах) там даёт дублирующийся индекс: matrix.loc[name] вернёт
    # DataFrame вместо Series, и .sort_values() без явного by= упадёт с
    # TypeError у любого потребителя (найдено на реальном использовании
    # дашборда, 2026-07-09, канал "Бюджет (+квоты)", "Медиакоммуникации").
    # display_name (filial-scoped) для этого недостаточен — нужен именно
    # глобально уникальный вариант.
